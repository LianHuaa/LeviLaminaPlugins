...
//创建文件夹
let json = new JsonConfigFile('./plugins/114514/PasswordLogin.js');
    ...
    if (json.get(player.name)["密码"] == null) {
        ...
        player.sendForm(cd, function(player, data) {
            ...
            if (data == null && data == undefined) {
                return loginWindow(player);
            } else if (data[0] != null && data[0] != "" && data[0] != undefined) {
                //   return loginWindow(player);
            } else {
                record["密码"] = data[0];
            ...
            if (json.get(player.name)["密码"] != null) {
                var cd1 = mc.newCustomForm();
                cd1.setTitle("登录窗口");
                cd1.addInput("输入密码");
                player.sendForm(cd1, function(player, data) {
                    if (data == null && data == undefined) {
                        return loginWindow(player);
                    } else if (data[0] != json.get(player.name)["密码"]) {
                        return loginWindow(player);
                    } else {
                        player.sendToast("提示", "登陆完成")
                    }
                }
                )
            }
